...
export default function AddEventScreen1({ onNext, onClose }) {
  ...
  const currentLabel = TIME_OPTIONS.find(o => o.id === timeOfDay).label;
  ...
  return (
    <div style={{
      ...
      {/* ── Top bar: chevron (right) · indicator (center) · X (left) ── */}
      <div style={{ position: 'relative', padding: '20px 20px 0' }}>
        <div style={{
          ...
          <button onClick={onClose} {...press('back')} style={{
            ...iconBtn,
        ...
        {/* 4-step indicator — centered relative to the full row, not the buttons */}
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'row', direction: 'rtl', gap: 6 }}>
            {[0, 1, 2, 3, 4].map(i => (
              <div key={i} style={{
                height: 8, width: i === 0 ? 24 : 8, borderRadius: 4,
                background: i === 0 ? '#323232' : 'rgba(50,50,50,0.25)',
              }} />
            ))}
          </div>
        </div>
          ...
          <div style={{
            ...
            {TIME_OPTIONS.map(({ id, label }) => {
              ...
              return (
                ...
                >
                  <Mandala timeOfDay={id} intensity={3} color="#323232" size={28} />
                  <span style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-start', padding: '0 24px 44px' }}>
        <button
          disabled={!confirmed}
          onClick={() => confirmed && onNext({ timeOfDay })}
